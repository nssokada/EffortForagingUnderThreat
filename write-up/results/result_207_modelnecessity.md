...
- **Cross-channel coupling** ([[result_402]]): ω, fit jointly, predicts anticipatory vigor at r ≈ 0.15–0.21 across all three threat levels in both samples, and W(u) further predicts the *absence* of an ω → vigor *slope* — also confirmed. This test is non-trivial only because ω and κ are shared between choice and vigor under W(u). Under M2 (population κ) the question reduces to "does choice-fit ω predict vigor by accident"; under M3 (collapsed θ) there are no separable parameters to test in the first place. The cross-channel prediction is a property of the embodied joint model, not a free fit.
...
- [[result_402]] — Cross-channel ω → anticipatory vigor; the inference that result 207 underwrites.

#!/usr/bin/env python3
"""
Affect & Metacognition analysis for the EVC 2+2 model (population epsilon).
...
Output:
  results/stats/evc_final_affect.csv
  results/stats/evc_final_metacognition.csv
  results/stats/evc_final_dissociation.csv
  results/figs/paper/fig_final_metacognition.png
"""
...
PARAM_F   = "/workspace/results/stats/oc_evc_final_params.csv"
POP_F     = "/workspace/results/stats/oc_evc_final_population.csv"
...
# With population epsilon: S = (1 - T^gamma) + epsilon * T^gamma * p_esc
feelings = feelings.merge(params[["subj", "c_effort", "c_death"]], on="subj", how="left")
n_before = feelings["subj"].nunique()
feelings = feelings.dropna(subset=["c_effort"])
n_after = feelings["subj"].nunique()
print(f"Subjects with params: {n_after}/{n_before}")

T = feelings["threat"].values
T_gamma = T ** GAMMA
feelings["S"] = (1 - T_gamma) + EPSILON * T_gamma * P_ESC
...
lmm_df.to_csv(f"{OUT_STATS}/evc_final_affect.csv", index=False)
print(f"\nSaved: {OUT_STATS}/evc_final_affect.csv")
...
beh = behavior.merge(params[["subj", "c_effort", "c_death"]], on="subj", how="inner")

T_b = beh["threat"].values
T_gamma_b = T_b ** GAMMA
S_b = (1 - T_gamma_b) + EPSILON * T_gamma_b * P_ESC

beh["S"] = S_b
# Choice equation: dEU = S*4 - ce*(0.81*D - 0.16)
beh["delta_eu"] = S_b * 4.0 - beh["c_effort"].values * (0.81 * beh["distance_H"].values - 0.16)
beh["ev_max_choice"] = (beh["delta_eu"] > 0).astype(int)
...
subj_metrics = subj_metrics.merge(params[["subj", "c_effort", "c_death"]], on="subj", how="inner")
...
pd.DataFrame([meta_results]).to_csv(f"{OUT_STATS}/evc_final_metacognition.csv", index=False)
print(f"\nSaved: {OUT_STATS}/evc_final_metacognition.csv")
...
print(f"  OVERALL: {'DOUBLE DISSOCIATION SUPPORTED' if dd_overall else 'NOT FULLY SUPPORTED'}")
...
dd_df.to_csv(f"{OUT_STATS}/evc_final_dissociation.csv", index=False)
print(f"\nSaved: {OUT_STATS}/evc_final_dissociation.csv")
...
clinical_short = {
    "OASIS_Total": "OASIS", "STAI_Trait": "STAI-T",
    "DASS21_Anxiety": "DASS-A", "PHQ9_Total": "PHQ-9", "STAI_State": "STAI-S",
}

cal_vals, dis_vals, labels = [], [], []
...
plt.savefig(f"{OUT_FIGS}/fig_final_metacognition.png", dpi=150, bbox_inches="tight", facecolor="white")
print(f"\nSaved: {OUT_FIGS}/fig_final_metacognition.png")
print("\nDone!")

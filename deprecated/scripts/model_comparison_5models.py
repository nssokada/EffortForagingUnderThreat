...
Vigor metric: normalized press rate (median(1/IPI) / calibMax), with cookie as covariate.
Previous version used excess effort (rate - req, cookie-centered) — preserved in
model_comparison_5models_excess.py.
...
def prepare_data():
    ...
    Vigor outcome: normalized press rate (median(1/IPI) / calibMax).
    Cookie type enters as a covariate in the model, not via centering.
    """
...
def vigor_eu_block(omega_v, T_w_v, epsilon, p_esc, sigma_motor, ce_vigor,
    ...
    Returns u_star on the normalized press rate scale [0.1, 1.5].
    """
...
def sample_vigor_pop_params():
    ...
    # Cookie intercept: absorbs baseline rate difference between heavy/light
    b_cookie = numpyro.sample('b_cookie', dist.Normal(0.0, 0.5))
    return epsilon, gamma, p_esc, sigma_motor, ce_vigor, sigma_v, b_cookie
...
def make_model_m2(N_S, N_choice, N_vigor):
    ...
    def model(ch_subj, ch_T, ch_dist_H, ch_choice,
              vig_subj, vig_T, vig_R, vig_req, vig_dist,
              vig_rate, vig_cookie):
        epsilon, gamma, p_esc, sigma_motor, ce_vigor, sigma_v, b_cookie = \
            sample_vigor_pop_params()
        tau_raw = numpyro.sample('tau_raw', dist.Normal(-1.0, 1.0))
        tau = jnp.clip(jnp.exp(tau_raw), 0.01, 20.0)
        ...
        # Vigor: predict normalized press rate
...
def make_model_m4(N_S, N_choice, N_vigor):
    ...
    def model(ch_subj, ch_T, ch_dist_H, ch_choice,
              vig_subj, vig_T, vig_R, vig_req, vig_dist,
              vig_rate, vig_cookie):
        # Vigor population params (no shared gamma/epsilon with choice)
        p_esc_raw = numpyro.sample('p_esc_raw', dist.Normal(0.0, 1.0))
        p_esc = jax.nn.sigmoid(p_esc_raw)
        sigma_motor_raw = numpyro.sample('sigma_motor_raw', dist.Normal(-1.0, 0.5))
        sigma_motor = jnp.clip(jnp.exp(sigma_motor_raw), 0.01, 1.0)
...
def make_model_m5(N_S, N_choice, N_vigor):
    ...
    def model(ch_subj, ch_T, ch_dist_H, ch_choice,
        ...
        # Vigor: predict normalized press rate directly
        T_w_v = jnp.power(vig_T, gamma)
        u_star = vigor_eu_block(omega[vig_subj], T_w_v, epsilon, p_esc,
                                sigma_motor, ce_vigor, vig_R, vig_req, vig_dist)
        rate_pred = u_star + b_cookie * vig_cookie
...
def evaluate_model(fit_result, data, n_samples=300, seed=44):
    ...
    if name == 'M1':
        ...
        return_sites = ['omega', 'epsilon', 'gamma', 'rate_pred']
    elif name == 'M3':
        return_sites = ['theta', 'epsilon', 'gamma', 'rate_pred']
    elif name == 'M4':
        return_sites = ['lam', 'omega', 'beta_pop', 'rate_pred']
    elif name == 'M5':
        return_sites = ['lam', 'omega', 'epsilon', 'gamma', 'rate_pred']

    return_sites.extend(['tau_raw', 'p_esc_raw', 'sigma_motor_raw',
                         'ce_vigor', 'sigma_v', 'b_cookie'])
...
# +1 for b_cookie in models with vigor
PARAM_COUNTS = {
    'M1': lambda N_S: N_S + 3,       # lam_raw + mu_lam, sigma_lam, tau_raw
    'M2': lambda N_S: N_S + 10,      # om_raw + mu_om, sigma_om, eps, gamma, tau, p_esc, sigma_motor, ce_vigor, sigma_v, b_cookie
    'M3': lambda N_S: N_S + 10,      # theta_raw + same pop params
    'M4': lambda N_S: 2*N_S + 12,    # lam+om raw + mu_lam, sigma_lam, mu_om, sigma_om, beta_pop, tau, p_esc, sigma_motor, ce_vigor, sigma_v, eps_vig, b_cookie
    'M5': lambda N_S: 2*N_S + 12,    # lam+om raw + mu_lam, sigma_lam, mu_om, sigma_om, eps, gamma, tau, p_esc, sigma_motor, ce_vigor, sigma_v, b_cookie
}
...
if __name__ == '__main__':
    ...
    print("  Vigor metric: normalized press rate (with cookie covariate)")
    print("=" * 70)

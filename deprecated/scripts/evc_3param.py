...
Architecture:
    ...
    Choice equation (k AND beta both enter):
        dEU = S * R_diff - k_i * effort(D_H, D_L) - beta_i * (1 - S)
        P(heavy) = sigmoid(dEU / tau)

        S = (1 - T^gamma) + epsilon * T^gamma * p_esc
        effort(D_H, D_L) = 0.81 * D_H - 0.16  (from LQR commitment cost)
        (1-S) captures threat: high threat + far distance = low S = high cost

        k captures: "avoid heavy because it's far/effortful" (distance main effect)
        beta captures: "avoid heavy because it's dangerous" (threat × distance interaction)
    ...
    Key insight: k and beta are identifiable because threat (T) and distance (D)
    are ORTHOGONAL in the 3x3x3 design. k captures distance-driven avoidance,
    beta captures threat-driven avoidance. S(T,D) gives beta leverage because
    both T and D affect survival probability.
"""
...
def prepare_data(behavior_rich_path, psych_path=None):
    """Load and prepare data for the 3-param model.
...
def make_model(N_S, N_choice, N_vigor):
    """Create the 3-param NumPyro model: k (effort) + beta (threat) in choice, cd in vigor."""
    ...
    def evc_3param(ch_subj, ch_T, ch_dist_H, ch_choice,
        ...
        # ── CHOICE: k handles effort, beta handles threat ──
        k_ch = k[ch_subj]
        beta_ch = beta[ch_subj]
        T_w_ch = jnp.power(ch_T, gamma)
        S_ch = (1.0 - T_w_ch) + epsilon * T_w_ch * p_esc

        # Heavy option: distance D_H, reward 5, effort = 0.81*D_H - 0.16
        # Light option: distance 1, reward 1, effort = 0.81*1 - 0.16 = 0.65
        # dEU = S * (R_H - R_L) - k * (effort_H - effort_L) - beta * (1-S) * ???
        #
        # For beta to differentiate H vs L through threat, we need (1-S) to
        # differ between the two options. Since S depends on distance through
        # escape probability, we compute S_H and S_L separately.
        #
        # S_H = (1 - T^gamma) + epsilon * T^gamma * p_esc_H
        # S_L = (1 - T^gamma) + epsilon * T^gamma * p_esc_L
        # where p_esc_H depends on distance (farther = harder to escape)
        # and p_esc_L is for the close/light cookie
        #
        # But in the original model, S doesn't depend on distance in choice
        # (distance enters through effort cost, not survival).
        # To give beta leverage, we need S to depend on which option is chosen.
        #
        # Alternative: beta penalizes threat EXPOSURE proportional to distance
        # dEU = S * 4 - k * effort_cost - beta * (1-S) * D_H
        # This captures: "under high threat (low S), I especially avoid FAR cookies"
        # beta * (1-S) * D_H vs beta * (1-S) * 1 for light
        # The differential is: beta * (1-S) * (D_H - 1)

        effort_cost = 0.81 * ch_dist_H - 0.16  # effort_H - effort_L (scales with D)
        # threat_cost: pure threat aversion, NO distance scaling
        # k captures: "I avoid far cookies" (effort/distance main effect)
        # beta captures: "I avoid cookies when threat is high" (threat main effect)
        # These are orthogonal because T and D are crossed in the design
        threat_cost = 1.0 - S_ch  # pure threat: high T → low S → high cost
        ...
        # ── VIGOR: cd drives survival incentive (unchanged) ──
...
def fit(data, n_steps=40000, lr=0.002, seed=42, print_every=5000):
    ...
    print(f"Fitting EVC 3-param (SVI, {n_steps} steps, lr={lr})")
...
def evaluate(fit_result, n_samples=500, seed=44):
    ...
    pred = Predictive(
        ...
        return_sites=['k', 'beta', 'c_death', 'epsilon',
    ...
    # BIC: 3 per-subject + population params
    # mu_k, sigma_k, mu_beta, sigma_beta, mu_cd, sigma_cd,
    # eps_raw, gamma_raw, tau_raw, p_esc_raw, sigma_motor_raw, ce_vigor_raw, sigma_v
    n_params = 3 * data['N_S'] + 13
    ...
    delta_eu = (S_ch * 4.0
                - k_vals[ch_subj_np] * effort_cost
                - beta_vals[ch_subj_np] * threat_cost)
    ...
    # Condition-level predictions (for diagnostics)
    ...
    print(f"\n{'=' * 60}")
    print(f"EVC 3-param Results (k + beta + cd)")
    print(f"{'=' * 60}")
    print(f"BIC: {bic:.0f}")
...
if __name__ == '__main__':
    ...
    # Save params
    out_path = 'results/stats/oc_evc_3param_params.csv'
    result['param_df'].to_csv(out_path, index=False)
    print(f"\nSaved: {out_path}")

    pop_path = 'results/stats/oc_evc_3param_population.csv'
    result['pop_df'].to_csv(pop_path, index=False)
    print(f"Saved: {pop_path}")

    cond_path = 'results/stats/oc_evc_3param_conditions.csv'

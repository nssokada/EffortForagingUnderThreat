...
The reallocation strategy approximated the normatively optimal policy. We computed the expected-value-maximizing choice for each trial given the task's reward and penalty structure, and found that participants matched the optimal policy on 69.8% of trials (SD = 12.0%). The dominant deviation was excessive caution: choosing the safe option when the risky option had higher expected value (2,813 trials) rather than the reverse (1,174 trials). Critically, deviation from optimality was strongly predicted by the model parameters: effort sensitivity $k$ increased suboptimality ($r = -0.69$, $p < .001$), reflecting over-avoidance of effort even when it was EV-positive. In contrast, both threat bias $\beta$ ($r = +0.44$) and vigor mobilization $\delta$ ($r = +0.66$; both $p < .001$) predicted *greater* optimality — individuals who reallocate effort more are closer to the normatively correct policy. Per-trial expected value loss averaged 0.54 points and was driven primarily by $k$ ($r = +0.56$) and offset by $\beta$ ($r = -0.62$). Psychiatric symptom measures did not predict optimality ($|r| < 0.07$).

A regression of total reward on all four parameters yielded $R^2 = 0.321$ ($F(4,288) = 34.04$, $p < .001$), with independent contributions from baseline vigor ($\alpha$: $t = 7.21$), danger mobilization ($\delta$: $t = 6.71$), and effort sensitivity ($k$: $t = -3.97$; all $p < .001$; Fig. 3f). Foraging success depends on *both* choosing wisely and executing vigorously.
...
This constitutes a near-optimal policy. A normative analysis showed that participants matched the expected-value-maximizing choice on 70% of trials, with deviations driven primarily by over-avoidance of effort ($k$). Individuals with stronger reallocation ($\beta$, $\delta$) were closer to the optimal policy — the coupling is not merely a behavioral regularity but an approximation of the EV-maximizing strategy. The coupling between $\beta$ and $\delta$ ($r = +0.32$ [0.23, 0.40] after propagating parameter uncertainty; $r = -0.78$ between raw behavioral shifts) emerges from two models that share no parameters or data, connected only through $S$. The complementary $k$–$\delta$ pattern ($r = -0.25$ [-0.32, -0.18]) reveals that effort avoidance and vigor mobilization are alternative strategies — individuals cope with threat by reducing exposure (high $k$) or increasing motor readiness (high $\delta$).

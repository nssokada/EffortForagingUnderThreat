...
Per-subject regressions of excess effort on danger yielded a population-mean slope of $\delta = +0.21$ (SD = 0.18), with 97.3% of participants showing positive slopes (Fig. 2a). Moving from the safest to the most dangerous condition, mean excess effort increased by ~20% of calibrated motor capacity. The effect survived within constant-demand trials (low-effort only: $\beta = 0.026$, $z = 3.54$, $p < .001$; Fig. 2b), ruling out a demand-driven artifact. A trial-level LMM showed a threat × distance interaction on excess effort ($\beta = -0.017$, $z = -2.12$, $p = .034$), paralleling the interaction structure of $S$ (Fig. 2c). Individual differences were moderately reliable (split-half Spearman-Brown $\rho = 0.451$).
...
As threat increased from $T = 0.1$ to $T = 0.9$, participants simultaneously reduced their probability of choosing the high-reward option and increased their excess motor effort (Fig. 3a). Across individuals, these shifts were anti-correlated ($r = -0.671$, $p < .001$; Fig. 3b). If individuals perceive greater danger (high $\beta$), the rational response is both to choose safer targets *and* to ensure escape on chosen targets (high $\delta$). We tested whether this coupling is a structural feature of the population by fitting a joint hierarchical Bayesian model.

The model estimated choice and vigor from a shared survival function $S$, with individual-difference parameters $[log(k_i), log(\beta_i), \alpha_i, \delta_i]$ drawn from a multivariate normal with a covariance matrix estimated via an LKJ Cholesky prior (see Methods). The off-diagonal elements directly estimate cross-domain coupling as a population parameter $\rho$, with posterior credible intervals.

We quantified the coupling at the individual-difference level using two complementary approaches. First, we estimated $\beta$ and $\delta$ from independent Bayesian hierarchical models — $\beta$ from choices alone, $\delta$ from excess effort alone — connected only through the survival function $S$ evaluated at the choice-estimated $\lambda$ (see Methods). Threat bias correlated with vigor mobilization ($r(log(\beta), \delta) = +0.55$, $p < .001$; Fig. 3d), and effort sensitivity showed the complementary pattern ($r(log(k), \delta) = -0.28$, $p < .001$; Fig. 3e). Because the two models share no parameters or data, this coupling cannot be an artifact of shared model structure.

Second, to confirm this coupling is a structural population feature and not merely a correlation between noisy point estimates, we fit a joint hierarchical model in which all four parameters $[log(k_i), log(\beta_i), \alpha_i, \delta_i]$ were drawn from a multivariate normal with a covariance matrix estimated via an LKJ Cholesky prior (Fig. 3c; see Methods). All six pairwise posterior correlations had 95% credible intervals excluding zero, and the model recovered meaningful individual differences in $\delta$ ($\sigma_\delta = 0.115$, 40.6% shrinkage from OLS; 99% of participants $\delta > 0$). The joint model also yielded a substantial ELBO improvement over choice-only ($\Delta$ELBO = +1,648), indicating that vigor observations provide information about the individual-difference structure beyond what choice data alone can identify.
...
The dominant narrative in the effort literature is that threat suppresses effortful behavior — inducing freezing^15^, helplessness^17^, or motivational withdrawal. Our data tell a different story. Under high threat, participants *increased* motor effort beyond task demands ($\delta > 0$ in 97% of participants) while *decreasing* the ambition of their choices. This is not avoidance — total effort may remain constant or increase. Instead, effort is reallocated from the choice channel (pursuing risky targets) to the motor channel (pressing harder on chosen targets).

This constitutes a rational policy. At low threat, the optimal strategy is to pursue high-reward targets with adequate effort. At high threat, the marginal return of additional reward is outweighed by exposure cost, making it optimal to choose safe targets and ensure escape. The coupling between $\beta$ and $\delta$ ($r = +0.55$ from independently estimated Bayesian parameters; $r = -0.78$ between raw behavioral shifts) is not a post-hoc observation — it emerges from two models that share no parameters or data, connected only through the survival function $S$. A joint hierarchical model with correlated random effects confirms the coupling as a structural population feature. The complementary $k$–$\delta$ pattern ($r = -0.28$) reveals that effort avoidance and vigor mobilization are alternative strategies — individuals cope with threat by reducing exposure (high $k$) or increasing motor readiness (high $\delta$).
...
**Choice models.** We compared five models, each testing one structural question (Supplementary Table 1): (M1) effort-only baseline, (M2) feature-based threat ($SV = R \cdot e^{-kE} - \beta \cdot T \cdot D$), (M3) multiplicative effort with exponential survival ($S = e^{-\lambda TD}$), (M4) multiplicative effort with hyperbolic survival ($S = (1-T) + T/(1+\lambda D)$), and (M5) additive effort with hyperbolic survival (winner: $SV = R \cdot S - k \cdot E - \beta \cdot (1-S)$). All models were fit using SVI with NumPyro^18^ (AutoNormal guide, 15,000 steps, Adam lr = 0.003). Per-subject $k_i$ and $\beta_i$ used non-centered parameterization with log-normal priors.

**Vigor model.** To estimate individual differences in danger-responsive vigor mobilization, we fit a separate hierarchical Bayesian model: $\text{excess}_{ij} = \alpha_i + \delta_i \cdot (1 - S_{ij}) + \varepsilon_{ij}$, where $S_{ij}$ uses the distance of the chosen option and $\lambda$ from the choice model ($\lambda = 13.9$). Per-subject $\alpha_i$ and $\delta_i$ had hierarchical normal priors. Fit via SVI (AutoNormal, 10,000 steps). The vigor model shares no parameters with the choice model — only the survival function $S$ (evaluated at the choice-estimated $\lambda$) provides the computational link. This separation ensures that cross-domain correlations between $\beta$ (from choice) and $\delta$ (from vigor) are not inflated by shared model structure.

**Joint model (robustness check).** As additional confirmation, we fit a joint hierarchical model with correlated random effects $\theta_i = [log(k_i), log(\beta_i), \alpha_i, \delta_i] \sim \mathcal{N}(\mu, \Sigma)$ where $\Omega \sim \text{LKJCholesky}(\eta = 2)$. Because vigor is more strongly modulated by threat probability than by escape distance, unconstrained $\lambda$ in the joint model is pulled higher than the choice-only estimate, and the LKJ correlation magnitudes depend on $\lambda$. The joint model confirmed the direction of all pairwise correlations across multiple $\lambda$ values (Supplementary Note 1).

### Statistical analysis

Trial-level effects: LMMs with random subject intercepts (statsmodels). Between-subject correlations: Pearson's $r$ on Bayesian posterior means from independently estimated models. Split-half reliability: odd/even with Spearman-Brown correction. All tests two-tailed, $\alpha = .05$.
...
**Fig. 2 | Danger drives excess motor vigor.** **a**, Distribution of per-subject danger-responsive vigor mobilization ($\delta$); 97.3% of participants show positive slopes. **b**, Excess effort as a function of danger within constant-demand trials, ruling out demand confounds. **c**, The threat × distance interaction in excess effort mirrors the survival function structure.

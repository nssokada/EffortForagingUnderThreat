**Hypothesis**
The overarching questions being asked are A) how do humans integrate energetic cost and predation risk when foraging, and B) does the metacognitive relationship between computed danger and experienced affect predict clinical anxiety symptoms?
...
The computational framework estimates two per-subject parameters from a joint hierarchical Bayesian EVC-LQR model:
...
The model uses an LQR (linear-quadratic regulator) cost structure where effort enters choice as a commitment cost (req² × D) and vigor as a deviation cost ((u − req)² × D).
...
2. The EVC-LQR model will jointly capture choice and vigor with two recoverable per-subject parameters
...
3. Subjective confidence will track choice quality but not motor execution quality
    a. Between-subject mean confidence will correlate positively with choice quality (proportion of EV-optimal choices), r > 0, p < 0.05
    b. Between-subject mean confidence will NOT correlate with survival rate, |r| < 0.10
    c. Steiger's test comparing the two dependent correlations (confidence-choice vs. confidence-survival) will be significant, p < 0.05
4. Metacognitive calibration and discrepancy will dissociate performance from clinical symptoms
    ...
    c. Discrepancy will predict clinical anxiety symptoms: r(discrepancy, STAI-State) > 0, p < 0.01
    d. Discrepancy will predict at least two additional clinical measures from {OASIS, STICSA, PHQ-9, DASS-Anxiety, DASS-Stress, DASS-Depression} at p < 0.05 uncorrected
...
2. (a–b) Per-subject predicted versus observed choice proportion and trial-level excess effort, from the jointly fitted EVC-LQR model. (c) Condition-level (threat × distance) predicted versus observed P(choose heavy). (d) Simulated-and-recovered parameter correlations in log space. (e) Pearson r between log(c_e) and log(c_d).
3. (a) Per-subject Pearson r between mean confidence and choice quality (proportion of trials matching the EV-maximizing option). (b) Per-subject Pearson r between mean confidence and survival rate (1 − capture rate). (c) Steiger's z comparing the two dependent correlations.
...
H2.
1. Fit the EVC-LQR model (2 per-subject parameters: c_e, c_d; population: γ, ε, c_e_vigor, τ, p_esc, σ_motor, σ_v) via NumPyro SVI (AutoNormal guide, 40,000 steps, Adam lr=0.002). Choice data: 45 choice trials (type=1). Vigor data: all 81 trials (types 1, 5, 6), with probe distances from startDistance. Cookie-type centering of excess effort using choice-trial means. The choice equation is ΔEU = S × 4 − c_e × (0.81 × D_H − 0.16); c_d does not enter choice (collinear with the reward differential). The vigor equation optimizes EU(u) = S(u) × R − (1 − S(u)) × c_d × (R + C) − c_e_vigor × (u − req)² × D via softmax-weighted grid search.
...
H3.
1. For each subject, compute mean confidence, choice quality (proportion of EV-optimal choices), and survival rate. Report Pearson r(confidence, choice quality) and r(confidence, survival rate).
2. Apply Steiger's test for dependent correlations (p < 0.05).
...
3. Test calibration → performance: r(calibration, choice quality) or r(calibration, survival), p < 0.05.
4. Test discrepancy → clinical: r(discrepancy, STAI-State) > 0, p < 0.01.
...
- Behavioral profile (c_e × c_d quadrant) analysis
...
**Name**
Expected Value of Control in effort foraging under threat: metacognitive bridges to clinical anxiety

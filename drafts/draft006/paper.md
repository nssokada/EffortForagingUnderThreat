...
where *S* is the subjective survival probability, *c*~e,i~ is the subject-specific effort cost, and the effort term reflects the commitment cost inspired by LQR theory---the squared required press rate scaled by distance (the difference in req^2^ $\times$ *D* between heavy and light options; specifically, 0.9^2^ $\times$ *D*~H~ - 0.4^2^ $\times$ 1 = 0.81*D*~H~ - 0.16). The survival function incorporates probability weighting and effort efficacy:
...
*Note.* ROPE = region of practical equivalence, |$\beta$| < 0.10. High percentage in ROPE (>80%) provides Bayesian evidence for the null; values in the 53--60% range are better characterized as inconclusive. All discrepancy HDIs exclude zero. STAI-Trait showed a negative association (*r* = -0.223), consistent with reduced trait anxiety in those who underestimate danger.

The model's computational parameters (*c*~e~ and *c*~d~) showed no reliable associations with symptom severity in either frequentist (all FDR-corrected *p* > .70) or Bayesian analyses (ROPE containment ranging 53--93%; Table 2). For ROPE containment above 80% (e.g., *c*~e~ for STAI-State at 91%, *c*~d~ for STAI-State at 93%), this constitutes positive Bayesian evidence for the null. For lower-ROPE cases (e.g., AMI at 53--54%), the evidence is better characterized as inconclusive. Overall, the pattern indicates that the computational parameters governing foraging behavior are predominantly dissociated from the affective dimension governing symptom severity.
...
We assessed whether any combination of model parameters and affective dimensions could predict individual symptom outcomes using cross-validated machine learning (elastic net and ridge regression with nested 10-fold CV, 5 repeats). All cross-validated *R*^2^ values were negative (range: -0.03 to -0.06), confirming that these associations are group-level patterns---useful for identifying mechanistic dimensions of variation---rather than individually predictive biomarkers.
...
The clean identification of the two parameters depends on a structural feature of our task design. In the choice comparison, the fixed capture penalty *C* cancels between options (since both share the same *C*), and the residual *c*~d~ term is collinear with the reward differential (*R*~H~ - *R*~L~), making *c*~d~ empirically unidentifiable from choice data. We therefore exclude *c*~d~ from the choice equation as a deliberate simplifying assumption. This means *c*~d~ is identified exclusively from vigor data---each parameter is pinned to its behavioral channel by the model's architecture rather than by post-hoc decomposition. We acknowledge that this exclusion means the model may misattribute some *c*~d~-driven choice variation to *c*~e~; however, the strong recovery of both parameters in simulation (*c*~e~: *r* = 0.92; *c*~d~: *r* = 0.94) and their near-independence (*r* = -0.14) suggest that this cross-contamination is minimal.

### Affective calibration as the bridge between computation and psychopathology

The most consequential finding is that the route from normative foraging computation to anxiety symptoms runs through the affect-computation relationship---through how people feel about danger---rather than through the decision parameters themselves. Neither *c*~e~ nor *c*~d~ predicted any symptom measure (all FDR-corrected *p* > .70; Bayesian ROPE containment 53--93%). Instead, discrepancy---the systematic excess of experienced anxiety over the model's danger signal---predicted symptoms of anxiety, depression, and stress across seven validated instruments ($\beta$ = 0.18--0.34).

We use the term "metacognitive" to describe calibration and discrepancy because these dimensions capture the alignment between a person's affective response and the computational danger signal that their own behavior implicitly reflects. This is not metacognition in the narrow sense of second-order confidence judgments about perceptual decisions^39^ (Fleming & Dolan, 2012), nor in the sense of Wells'^21^ meta-worry (beliefs about one's own worry). Rather, we invoke metacognition in a broader sense: the monitoring of one's own internal states relative to environmental demands. Calibration indexes how faithfully a person's anxiety tracks the survival-relevant danger that their choices reveal they have computed; discrepancy indexes the systematic bias in that monitoring. This usage is closer to Paulus and Stein's^22^ interoceptive prediction error framework than to the perceptual metacognition tradition, and we acknowledge that "affective calibration" and "affective bias" are equally valid descriptors. We retain "metacognitive" because the key theoretical point is that the clinically relevant dimension is not the computation itself but the person's *relationship to* their computation---a second-order property.

This finding provides computational specificity to theories linking affect-computation mismatch to anxiety^21,22^. Wells^21^ proposed that clinical anxiety reflects dysfunctional beliefs *about* threat cognition (meta-worry), not simply elevated threat estimation. Paulus and Stein^22^ formalized a related idea as interoceptive prediction error: anxiety disorders involve systematic mismatch between expected and actual body states under threat. Our discrepancy measure operationalizes this mismatch within a normative framework: it quantifies how much a person's anxiety exceeds what the model's survival computation warrants, given the same threat environment that all participants face. We note that our measure is not a true interoceptive prediction error---it is the residual between a self-reported affective state and a model-derived environmental quantity, not a mismatch between predicted and actual body states. The connection to Paulus and Stein is therefore analogical rather than literal. The orthogonality of calibration and discrepancy (*r* = 0.019) means that a person can accurately track danger (high calibration) while simultaneously reporting more anxiety than that danger warrants (high discrepancy)---capturing the phenotype of someone who "knows" they are safe but "feels" afraid.

The calibration-performance association (*r* = 0.19--0.23) complements this picture. Participants whose anxiety faithfully tracked computed danger made better foraging decisions and survived more often, suggesting that well-calibrated anxiety functions as an adaptive internal signal---an alarm system tuned to actual threat that enables appropriate behavioral regulation. This aligns with functional accounts of anxiety^32^ and with Wise and colleagues' finding that confidence tracks the quality of internal models of threat^33^. The critical addition of our work is showing that this adaptive monitoring function (calibration) is orthogonal to the symptom-relevant dimension (discrepancy), and that it is the *mismatch* between computation and affect, not the computation itself, that predicts symptom severity. We characterize this as a differential prediction pattern rather than a double dissociation in the strict neuropsychological sense, because the separation is not absolute: calibration showed a weak positive association with STAI-State (*r* = 0.12, *p* = .04), and discrepancy showed a modest negative association with survival (*r* = -0.15, *p* = .009). These cross-associations are small relative to the primary effects and do not undermine the overall pattern, but they indicate leakage between the two dimensions that a true double dissociation would preclude.
...
The associations with anxiety symptoms we report are statistically robust but modest in magnitude ($\beta$ = 0.18--0.34, corresponding to approximately 3--11% of symptom variance). These effect sizes are consistent with the broader computational psychiatry literature^24,33^ and with the inherent challenge of linking single-task behavioral parameters to multidetermined psychiatric constructs measured by self-report in a non-clinical sample. Four honest constraints are important to note.
...
**Population parameters:** $\gamma$ = 0.209 (probability weighting); $\varepsilon$ = 0.098 (effort efficacy); *c*~e,vigor~ = 0.003 (deviation cost, LQR-inspired); $\tau$ = 0.476 (choice temperature); *p*~esc~ (escape probability); $\sigma$~motor~ (motor noise); $\sigma$~v~ (vigor observation noise).

**Choice model.** $\Delta$EU = *S* $\times$ 4 - *c*~e,i~ $\times$ (0.81*D*~H~ - 0.16). The capture aversion *c*~d~ is excluded from the choice equation because its contribution to the option differential is collinear with the reward term, making it empirically unidentifiable from choice data (see Results for derivation). P(heavy) = sigmoid($\Delta$EU / $\tau$).
...
NumPyro stochastic variational inference (SVI) with an AutoNormal guide (mean-field approximation), Adam optimizer (lr = 0.002), 40,000 steps. We used SVI with a mean-field approximation, which provides point estimates of posterior means but may underestimate posterior uncertainty due to the assumption of posterior independence between parameters. HDIs and ROPE analyses reported for the clinical regressions (which use full MCMC via bambi/PyMC) are not affected by this approximation, but the population-level parameter estimates ($\gamma$, $\varepsilon$, $\tau$) from the main model should be interpreted as approximate posterior modes rather than full posteriors. BIC = 2 $\times$ loss + *k* $\times$ log(*n*), where *k* = 2 $\times$ *N*~subjects~ + number of population parameters. We note that using the ELBO loss in place of the log-likelihood for BIC computation is non-standard, as the ELBO is a lower bound on the marginal likelihood. This may affect model comparison, particularly for hierarchical models where the effective number of parameters is less than the raw count due to shrinkage. Future work should supplement BIC with WAIC or LOO-CV, which properly account for effective parameter count.
...
*Note.* Median split is descriptive; these profiles are not statistically validated as a typology. The Vigilant profile's earnings advantage demonstrates that combining willingness to forage with vigorous execution yields the best outcomes---a pattern that may not generalize beyond this task structure.
